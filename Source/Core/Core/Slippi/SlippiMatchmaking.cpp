...
{
	// ⚠️ NOT CALLED. Left here with its teeth pulled because the idea is right
	// and the guard was not.
	//
	// Turning this on killed live matches about ten seconds in - one heartbeat.
	// The only thing standing between a PLAYER and watching their own game is
	// PeppyInActivePair, which compares our configured name against the roster.
	// One tick where that comparison does not hold - an empty roster, a name
	// that came back trimmed - and the player's own client starts watching
	// itself, its pads get driven from the timeline, and the match dies.
	//
	// A name comparison is far too weak a thing to hang that on. It needs a
	// guard that cannot be wrong: whether this client has a netplay match of
	// its own in progress.
	(void)resp;
}
